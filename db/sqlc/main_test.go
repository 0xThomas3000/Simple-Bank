...
func TestMain(m *testing.M) {
	var err error
	testDB, err = sql.Open(dbDriver, dbSource) // Create a new connection to the DB, returns a connection obj and an Error
	if err != nil {
		log.Fatal("cannot connect to db:", err)
	}

	testQueries = New(testDB) // Use a connection to create a new testQueries object
